...
// Efficient Roguelike FOV algorithm
// unimplemented for project because
// in the current state of algorithm, the walls need to points on the grid (just like walkable tiles)
...
public sealed class Visibility
{
    ...
    public void Compute(IntVector2 origin, int rangeLimit)
    {
    ...
    void Compute(uint octant, IntVector2 origin, int rangeLimit, uint x, Slope top, Slope bottom)
    {
    ...
    bool BlocksLight(uint x, uint y, uint octant, IntVector2 origin)
    {
    ...
    void SetVisible(uint x, uint y, uint octant, IntVector2 origin)
    {
        uint nx = (uint)origin.x, ny = (uint)origin.y;
        switch (octant)
        {
