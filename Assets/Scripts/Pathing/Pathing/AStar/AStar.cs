...
public class AStar
{
    ...
    public List<MazeCell> GetPath(PathLayer layer, MazeCell start, MazeCell end, int forcedGraphIndex)
    {
        // clearing dictionaries for re-use
        cameFrom.Clear();
        costSoFar.Clear();
    ...
    public List<MazeCell> GetPath(PathLayer layer, MazeCell start, MazeCell end, List<int> avoidIndices)
    {
        // clearing dictionaries for re-use
        cameFrom.Clear();
        costSoFar.Clear();
        ...
        while (frontier.Count > 0)
        {
            ...
            foreach (var next in current.connectedCells)
            {
                var indices = next.GetGraphAreaIndices();

                if (next.state > 1 || (indices.Count == 1 && avoidIndices.Contains(indices[0])))
                    continue;
        ...
        {
            string str = "";
            str += "no available path from " + start + " to " + end + " while avoiding listed indices";
